...
pub async fn subscribe(
    ...
    .await
    .map_err(|e| {
        SubscribeError::UnexpectedError(Box::new(e), "Failed to send a confirmation mail.".into())
    })?;
...
async fn send_confirmation_email(
    ...
    let html_body = templates
        ...
        .map_err(|e| {
            SubscribeError::UnexpectedError(
                Box::new(e),
                "Error rendering html email template.".into(),
            )
        })?;
    ...
    let plain_body = templates
        ...
        .map_err(|e| {
            SubscribeError::UnexpectedError(
                Box::new(e),
                "Error rendering plain text email template.".into(),
            )
        })?;
    ...
    email_client
        ...
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(e), "Error sending email".into()))?;
